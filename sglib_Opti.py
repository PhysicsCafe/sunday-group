...
# --------------------------------------------------------------------------
#
# September 2018: THIS FILE IS CURRENTLY UNDERGOING A CHANGE TO PYTHON 3
#
# Notes on Python3 conversion::
#
# Sept 26, 2018: I'm working through: python-future.org/compatible_idioms.html
#   - I did everything but ...
#   - It's possible that problems with division will have to be debugged
#   - May have problems with class Arrow3D(FancyArrowPatch)
#   - Not doing anything about "unicode literals" OR 'byte-string literals'
#   - I don't understand what they're talking about in
#     "Imports relative to a package"
#   - Don't understand range(n) vs list(range(n)). Not changing for now.
#
# --------------------------------------------------------------------------
#
# General Notes on sglib:
#
# - January 2018: his is the newest sglib / no more sglibN.
#
...
#    2.0 Bit String functions
#    2.x Other utility code
#
# 3. Linear Algebra Routines
#    Various basic things
#    Factor a state 
#    Separate high bits of an entangled state / format it
#    3.4 Factor X and Y bits / Plot X against Y
#
# 3A. Graphics (need to move)
#
...
# makeBitString(5, 4) returns '0101'
def makeBitString(value, N):
    fmt = '{0:0%sb}'%N
    return fmt.format(value)

# Takes a bit string and returns an integer
def BitStringToInt(bits):
    return int(bits, 2)

# Does an exclusive OR on a bit string
# strOR('101', '110') returns '111'
#
# I'm not sure the 'string or' is really a good idea. I think I'm
# doing this better with integers somewhere in the various shor
# notebooks. In fact all of the 'str' routines are questionable and
# should at least be reconsidered as integer functions.
#
def strOR(a,b):
    result = ''
    for n in range(len(a)): result += str( int(a[n]) ^ int(b[n]) )
    return(result)

# Returns all possible bit patterns
# makeBitPatterns(2) returns [ '00', '01', '10', '11' ]
def makeBitPatterns(N):
    fmt = '{0:0%sb}'%N
    patterns = []
    for n in range(2**N): patterns.append(fmt.format(n))
    return patterns
...
# DBG()
#
# This is good to use when you have a long-running function that
# wants to output status occasionally (because of the flush).
#
def DBG(msg):
...
# Various functions for dealing with "kets"
#
# This probably be changed (and generalized) as ways of
# dealing with "states" rather than "kets." And there is 
# other code, other places in this file for doing things with
# states. I should gather is all together. Not here, but in a
# "major" section of its own.
#
# There are also a couple of other "ket" routines in the electron
# spin code. But to move it I also need to mover around some
# definitions.
#
# In the meantime, what I'm doing is gathering up some ket stuff
# that's used all over the place (at least in this directory and
# in the Shor directory).
#
# the reason that these functions typically have "two names" is
# so that I don't have to edit all the code in all the notebooks
# that already use the short names.

# Make a ket (column matrix) or bra (row matrix) with the integer
# value n, consisting of num_bits.
#
def ket_from_number(n, num_bits):
    from sympy import Matrix, zeros
    ket = Matrix((zeros(2**num_bits, 1)))
    ket[n] = 1
    return ket
ket_n = ket_from_number

def bra_from_number(n, num_bits): return ket_n(n, num_bits).adjoint()
bra_n = bra_from_number

# Returns classical number only from ket
def ket_to_number(ket):
    found = 0
    ...
    if found == 1: return n
        ...
        print('NOT A CLASSICAL STATE')
        return None
n_from_ket = ket_to_number

# Make a printable ket with a number in it from a vector
def pket(state):
    n = ket_to_number(state)
    return ket(n)

# These are latex ket and bra displays
def ket(label): return r'\left|\, %s \right\rangle'%label
def bra(label): return r'\left\langle %s \,\right|'%label
...
class format:
    # Format things, depending on the "exact" flag.
    #
    ...
    def fix(self, n):
        if sy.im(n) != 0: 
            n = complex(n)
            n = round(n.real, self.ndigs) + round(n.imag, self.ndigs)*1j
        else: 
            n = float(n)
            n = round(n, self.ndigs)
        return(n)
    ...
    def do_format(self, M):
        ...
        # For a matrix (and hopefully a larger structure, but I'm not sure)
        Mc = M.applyfunc(self.fix)
        return(myltx(Mc))
...
class sg_format_state:
    ...
    def format(self, state, which_ltx='mine', D=False):
        ...
        for n in range(len(state)):
            ...
            if shape(state)[0] > 1:
                string += r'\left |' + T[n] + r'\right \rangle'
            else:
                string += r'\left \langle' + T[n] + r'\right |'
        return(string)
...
def myltx(obj, V=False):
    ...
    try:
        ...
        if type(gcd) != type(1/sqrt(2)): return(sy.latex(obj))
        #obj = obj*gcd
        #return( myltx(1/gcd) + myltx_frac(obj) )
        if gcd < 0:
            gcd = -gcd
        obj = obj/gcd
        return( myltx(gcd) + myltx_frac(obj) )
...
def myltx_frac(frac):
    ...
    # If it's negative we'll remember that and make it positive here
    was_neg = False
    if frac < 0:
        frac = -frac
        was_neg = True
    ...
    #print('one: %s'%one),; print('two %s'%two)

    if one == sy.Rational(1,2) and two == sy.sqrt(2):
        if was_neg: return(r'-\frac{1}{\sqrt{2}}')
        return(r'\frac{1}{\sqrt{2}}')
    if one == sy.Rational(1,3) and two == sy.sqrt(3):
        if was_neg: return(r'-\frac{1}{\sqrt{3}}')
        return(r'\frac{1}{\sqrt{3}}')
    if one == sy.Rational(1,5) and two == sy.sqrt(5):
        if was_neg: return(r'-\frac{1}{\sqrt{5}}')
        return(r'\frac{1}{\sqrt{5}}')
    if one == sy.Rational(1,6) and two == sy.sqrt(6):
        if was_neg: return(r'-\frac{1}{\sqrt{6}}')
        return(r'\frac{1}{\sqrt{6}}')
    if one == sy.Rational(1,7) and two == sy.sqrt(7):
        if was_neg: return(r'-\frac{1}{\sqrt{7}}')
        return(r'\frac{1}{\sqrt{7}}')
    if one == sy.Rational(1,4) and two == sy.sqrt(2):
        if was_neg: return(r'-\frac{1}{\sqrt{8}}')
        return(r'\frac{1}{\sqrt{8}}')
...
#
# Separate the high bits of an entangled state
#
#   This won't always work? (i.e. for complex amplitudes?)
#
def separate_high_bits(vector, n_high_bits):
    from sympy import log, sqrt
    N = log(len(vector), 2)
    n_low_bits = N - n_high_bits
    
    highs = [] # List of non-zero amplitude high bit patterns
    lows  = [] # Each is a list of low bit patterns for each member of highs

    # Look at each (non-zero) element of the state
    for n in range(len(vector)):
        if vector[n] == 0: continue
        bits = makeBitString(n, N)
        hbits = bits[:n_low_bits]
        lbits = bits[n_low_bits:]
        amp = vector[n]

        # Try to find the high bit pattern on the list
        try:
            index = highs.index(hbits)
        except ValueError:
            highs.append(hbits) # Add high bit pattern to highs
            lows.append([]) # Add an empty list to lows
            index = highs.index(hbits)

        # Add in this particular low bit pattern with it's amplitude
        lows[index].append( [amp, lbits] )

    # Now try to adjust the amplitudes
    # Only do this if they are all "equal" (in terms of probabilities)
    all_equal = True
    amp = lows[0][0][0]
    amp = amp*amp.conjugate()
    for low_list in lows:
        for low in low_list:
            if low[0]*low[0].conjugate() != amp:
                all_equal = False
                break
  
    new_highs = [] 
    if all_equal:
        high_amps = 1/sqrt(len(highs))
        for n in range(len(highs)):
            new_highs.append([high_amps , highs[n]])
            for low in lows[n]:
                low[0] = low[0]/high_amps
    else:
        for high in highs:
            new_highs.append([1, high])

    return new_highs, lows

#
# format_entangled_1
#
#   Format an entangled state using "separate_high_bits" above
#
def format_entangled_1(state, n_high_bits, line_breaks=False):
    highs, lows = separate_high_bits(state, n_high_bits)
    p_string = '$'
    for n in range(len(highs)):
        if n != 0 and highs[n][0] >= 0: p_string += ' + '
        p_string += myltx(highs[n][0]).replace(r'\frac{-', r'-\frac{')
        p_string += r'\left|' + highs[n][1] + r'\right\rangle'
        p_string += r'\otimes \left('
        for m in range(len(lows[n])):
            low = lows[n][m]
            if m != 0 and low[0] >= 0: p_string += ' + '
            p_string += myltx(low[0]).replace(r'\frac{-', r'-\frac{')
            p_string += '\left|' + low[1] + r'\right\rangle'
        p_string += r'\right)'
        if line_breaks: p_string += r'\\' # Note: \\ is latex line break
    p_string += '$'
    return p_string

#-------------------------------------------------------------------#
#                                                                   #
#                   3.4 Factor XY / Plot XY                         #
#                                                                   # 
#-------------------------------------------------------------------#

# factor_xy()
#
# The idea here is that we have a state consisting of "x bits" and
# "y bits" and we want to "separate" the two sets of bits. We are not
# literally factoring into two states. In fact the composite state
# is likely to be entanged. We are simply separating the x and y values
# to that we can, for example, plot x against y.
#
# For some reason we have trouble simplifying some zero values.
# This led to a number of complications and numerical approximations.
#
# Note: You can give N=-1 to get *all* the values.
#
def factor_xy(product, bits1, bits2, gt=10**-99, N=5, y=None):
    from sys import stdout
    ...
    prod_index = 0
    n_returned = 0
    for n1 in range(2**bits1):
        if time() - start_time > 15:
            start_time = time()
        for n2 in range(2**bits2):
            amp = product[prod_index] # probability amplitude
            amp = complex(amp)
            P = ( amp*amp.conjugate() ) # probability
            if P.real > gt:
                if y==None:
                    ret_vec.append([prod_index, product[prod_index], n1,n2])
                    n_returned += 1
                else:
                    if n2==y:
                        ret_vec.append([prod_index, product[prod_index], n1,n2])
                        n_returned += 1
                if n_returned == N: return ret_vec
            prod_index += 1
    return ret_vec

# plot_xy()
#
# This takes the vector returned by factor_xy() and plots x against y.
#
def plot_xy(rvec, title=None, limit=None, V=False, ticks=None):
    import matplotlib.pyplot as plt
    X = []; Y = []
    if limit == None: R = len(rvec)
    else: R = limit
    for n in range(R):
        element = rvec[n]
        X.append(element[2])
        Y.append(element[3])
    fig, ax = plt.subplots()
    ax.bar(X,Y, width=.5, align='center')
    # element[2] should still have the value of max x encountered
    ax.set_xlim(-.5, element[2] + .5)
    if ticks != None: ax.set_xticks(ticks, minor=False)
    fig.set_size_inches(10,4)
    if title != None: ax.set_title(title, fontsize=20)

# plot_xy_values()
#
# This version can do some extra things. I *think* it could potentially
# replace plot_xy() but I'd have to look at that more closely.
#
def plot_xy_values(x, y,
        low=None, high=None, title=None, width=None, do_ticks=-1,
        figsize=[10,4]):

    from matplotlib import pyplot as plt
    import uuid

    if width==None: width=.5
    if low == None: low = 0
    else: low = int(low)
    if high == None: high = int(x[-1])
    else: high = int(high)

    # If the do_ticks flag is zero or greater, then we want to provide
    # x ticks when values of y are >= do_ticks.
    #ticks = [low]
    ticks = []
    for n in range(len(x)):
        if y[n] > do_ticks: ticks.append(x[n])
    #ticks.append(high)

    fig, ax = plt.subplots()

    # The idea here is to only plot from low to high
    # This DOES save plotting time, which can be substantial
    #ax.bar(x,y, width=width, align='center')
    ax.bar(x[low:high+1],y[low:high+1], width=width, align='center')

    if do_ticks >= 0: ax.set_xticks(ticks, minor=False)
    fig.set_size_inches(figsize[0], figsize[1])
    ax.set_xlim(low-width, high+width)
    if title != None: ax.set_title(title, fontsize=20)

    # The point of this next part is to put the plot where you want
    # it, rather than at the end of the input cell. This is only going
    # to work in my particular enviornment with a pre-created directory
    # to store these things.
    fig_name = '01_TEMP_PLOT_DIR/' + str(uuid.uuid4()) + '.png'
    ax.figure.savefig(fig_name, bbox_inches='tight')
    img_string = '<img src="' + fig_name + '">'
    Print(img_string)
    plt.close(fig)

    # In case the caller wants to do something with the plot.
    return fig, ax

#####################################################################
#                                                                   #
#                   3A. Graphics (need to move these)               #
#                                                                   #
#####################################################################
...
def ket_from_symbol(state):
    ...
    print('Error: ket_from_symbol() got unknown state id: %s'%state)

def ket_to_symbol(state):
    #Print('ket_to_symbol() got: $%s$'%latex(state))
    state.simplify()
    if state == pZ: var = '+z'
    elif state == mZ: var = '-z'
    elif state == pX: var = '+x'
    ...
    else: Print('Error in ket_to_symbol()')
...
class electron_problem:
    ...
    def new(self):
        ...
        k1 = ket_from_symbol(+ibasis); k2 = ket_from_symbol(-ibasis)
        ...
        if alpha_ltx != '0': msg += alpha_ltx + ket_to_symbol(k1)
        if beta_ltx != '0': msg += beta_ltx + ket_to_symbol(k2)
        msg += '$'
        msg += '\n<br><br>\n'
        ...
        ostate = ket_from_symbol(eval*obasis)
...
def superposition_spin_problem(psi, meas_basis, quiet=False):
    ...
    P('states given, multiplying them by the given probability amplitudes,')
